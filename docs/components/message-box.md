...
`MessageBox` 是一個帶有邊框與 icon 的訊息提示容器，支援 info、danger、success、warning、default 五種型態，icon 可自訂。
...
<SHMessageBox type="warning">這是一個 warning 訊息</SHMessageBox>
...
| 名稱 | 類型                                  | 預設值 | 說明                      |
| ---- | ------------------------------------- | ------ | ------------------------- |
| type | 'info'\|'danger'\|'success'\|'warning'\|'default' | 'info' | 訊息盒型態                |
